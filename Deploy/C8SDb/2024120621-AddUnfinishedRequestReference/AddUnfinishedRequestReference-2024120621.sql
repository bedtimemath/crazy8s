...
IF NOT EXISTS (
    SELECT * FROM [__EFMigrationsHistory]
    WHERE [MigrationId] = N'20241206214650_AddUnfinishedRequestReference'
)
BEGIN
    ALTER TABLE [Unfinisheds] ADD [RequestId] int NULL;
END;
GO
...
BEGIN
    ALTER TABLE [Unfinisheds] ADD CONSTRAINT [FK_Unfinisheds_Requests_RequestId] FOREIGN KEY ([RequestId]) REFERENCES [Requests] ([ApplicationId]);
END;
GO
...
BEGIN
    INSERT INTO [__EFMigrationsHistory] ([MigrationId], [ProductVersion])
    VALUES (N'20241206214650_AddUnfinishedRequestReference', N'8.0.10');
END;
GO
