...
IF NOT EXISTS (
    ...
    WHERE [MigrationId] = N'20241111172705_InitialSetup'
)
BEGIN
...
BEGIN
    CREATE TABLE [Unfinisheds] (
        ...
        [EndPart05On] datetimeoffset NULL,
...
BEGIN
    ...
    VALUES (N'20241111172705_InitialSetup', N'8.0.10');
END;
GO
...
IF NOT EXISTS (
    SELECT * FROM [__EFMigrationsHistory]
    WHERE [MigrationId] = N'20241116174615_RemoveEndPart05'
)
BEGIN
    ...
    WHERE ([d].[parent_object_id] = OBJECT_ID(N'[Unfinisheds]') AND [c].[name] = N'EndPart05On');
    IF @var0 IS NOT NULL EXEC(N'ALTER TABLE [Unfinisheds] DROP CONSTRAINT [' + @var0 + '];');
    ALTER TABLE [Unfinisheds] DROP COLUMN [EndPart05On];
END;
GO
...
BEGIN
    INSERT INTO [__EFMigrationsHistory] ([MigrationId], [ProductVersion])
    VALUES (N'20241116174615_RemoveEndPart05', N'8.0.10');
END;
GO
