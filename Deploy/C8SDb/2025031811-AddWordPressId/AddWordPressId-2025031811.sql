...
IF NOT EXISTS (
    ...
    WHERE [MigrationId] = N'20250311175515_InitialSetup'
)
BEGIN
...
BEGIN
    CREATE TABLE [Requests] (
        ...
        [FullSlateAppointmentId] bigint NULL,
        [FullSlateAppointmentStartsOn] datetimeoffset NULL,
...
BEGIN
    ...
    VALUES (N'20250311175515_InitialSetup', N'9.0.2');
END;

IF NOT EXISTS (
    SELECT * FROM [__EFMigrationsHistory]
    WHERE [MigrationId] = N'20250316193547_UpdateClubOrderRelations'
)
BEGIN
    DROP INDEX [IX_Orders_ClubId] ON [Orders];
END;
...
BEGIN
    INSERT INTO [__EFMigrationsHistory] ([MigrationId], [ProductVersion])
    VALUES (N'20250316193547_UpdateClubOrderRelations', N'9.0.2');
END;

IF NOT EXISTS (
    SELECT * FROM [__EFMigrationsHistory]
    WHERE [MigrationId] = N'20250318113407_AddWordPressId'
)
BEGIN
    DECLARE @var sysname;
    SELECT @var = [d].[name]
    ...
    WHERE ([d].[parent_object_id] = OBJECT_ID(N'[Persons]') AND [c].[name] = N'WordPressUser');
    IF @var IS NOT NULL EXEC(N'ALTER TABLE [Persons] DROP CONSTRAINT [' + @var + '];');
    ALTER TABLE [Persons] DROP COLUMN [WordPressUser];
END;
...
BEGIN
    ALTER TABLE [Persons] ADD [WordPressId] int NULL;
END;
...
BEGIN
    INSERT INTO [__EFMigrationsHistory] ([MigrationId], [ProductVersion])
    VALUES (N'20250318113407_AddWordPressId', N'9.0.2');
END;
