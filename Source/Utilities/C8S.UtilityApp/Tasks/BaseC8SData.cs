...
internal abstract class BaseC8SData(
    IDbContextFactory<C8SDbContext> dbContextFactory,
    OldSystemService oldSystemService)
    : IActionLauncher
{
    #region Constants & ReadOnlys
    protected const string SkuTable = "Sku";
    protected const string ClubTable = "Club";
    protected const string OrderTable = "Order";
    protected const string OrderSkuTable = "OrderSku";
    protected const string ApplicationTable = "Application";
    protected const string ApplicationClubTable = "ApplicationClub";
    protected const string CoachTable = "Coach";
    protected const string OrganizationTable = "Organization";
    protected const string UserTable = "User";
    protected const string CompanyTable = "Company";
    protected const string PostalAddressTable = "PostalAddress";
    protected const string UsaPostalTable = "UsaPostal";

    protected const int SaveBlock = 500;

    protected readonly IDbContextFactory<C8SDbContext> DbContextFactory = dbContextFactory;
    protected readonly OldSystemService OldSystemService = oldSystemService;
    #endregion

    #region Abstract Methods
    public abstract Task<int> Launch(); 
    #endregion

    #region Get Entities
    protected async Task<List<int>> GetPersonIdsForPermissions()
    {
    ...
    #region Add Entities
    protected async Task<int> CreateOffersFromSkus(List<SkuSql> sqlSkus)
    {
    ...
    protected async Task<int> CreateKitsFromSkus(List<SkuSql> sqlSkus, OfferLookup offerIdLookup)
    {
        var skusCount = sqlSkus.Count;
        var added = 0;
    ...
    protected async Task<(int, int)> AddPersons(List<CoachSql> sqlCoaches)
    {
        var added = 0;
        var dupes = 0;
    ...
    protected PersonDb CreatePersonFromCoach(CoachSql sqlCoach) => new ()
        {
    ...
    protected async Task<int> AddPlaces(List<OrganizationSql> sqlOrganizations, List<AddressSql> sqlAddresses)
    {
    ...
    protected async Task<int> AddTickets(List<ApplicationSql> sqlApplications, List<ApplicationClubSql> sqlApplicationClubs)
    {
        ...
        for (int index = 0; index < sqlApplicationsCount; index++)
        {
            ...
            var request = new RequestDb()
            {
                ...
                AppointmentStartsOn = sqlApplication.ScheduledCall,
    ...
    protected async Task<int> AddOrders(List<OrderSql> sqlOrders, List<OrderTrackerSql> sqlOrderTrackers, List<AddressSql> sqlAddresses)
    {
    ...
    protected async Task<(int, int)> AddClubs(List<ClubSql> sqlClubs,
    ...
    protected async Task<(int,int)> AddKitPages()
    {
        await using var dbContext = await DbContextFactory.CreateDbContextAsync();
    ...
    protected async Task<(int, int)> AddPermissions(List<int> personIds)
    {
    ...
    #region Joins
    protected async Task<(int,int)> JoinPersonsWithPlaces()
    {
        await using var dbContext = await DbContextFactory.CreateDbContextAsync();
    ...
    protected async Task<(int, int, int)> JoinPersonsToTickets(List<ApplicationSql> sqlApplications,
    ...
    protected async Task<(int, int, int)> JoinPlacesToTickets(List<ApplicationSql> sqlApplications, List<AddressSql> sqlAddresses,
    ...
    protected async Task<int> JoinOffersToOrders(List<OrderSkuSql> sqlOrderSkus,
    ...
    protected async Task<(int, int)> JoinPersonsToClubs(List<ClubSql> sqlClubs, ClubLookup clubIdLookup, PersonLookup personIdLookup)
    {
        await using var dbContext = await DbContextFactory.CreateDbContextAsync();
    ...
    #region Remove Dupes
    protected async Task RemoveExistingOrganizations(List<OrganizationSql> sqlOrganizations)
    {
        await using var dbContext = await DbContextFactory.CreateDbContextAsync();
    ...
    protected async Task RemoveExistingCoaches(List<CoachSql> sqlCoaches)
    {
        await using var dbContext = await DbContextFactory.CreateDbContextAsync();
        var existingGuids = await dbContext.OldNews
    ...
    protected async Task RemoveExistingTickets(List<ApplicationSql> sqlApplications)
    {
        await using var dbContext = await DbContextFactory.CreateDbContextAsync();
        var existingGuids = await dbContext.OldNews
    ...
    protected async Task RemoveExistingSkus(List<SkuSql> sqlSkus)
    {
        await using var dbContext = await DbContextFactory.CreateDbContextAsync();
        var existingGuids = await dbContext.OldNews
    ...
    protected async Task RemoveExistingOrders(List<OrderSql> sqlOrders)
    {
        await using var dbContext = await DbContextFactory.CreateDbContextAsync();
        var existingGuids = await dbContext.OldNews
    ...
    protected async Task RemoveExistingOrderSkus(List<OrderSkuSql> sqlOrderSkus)
    {
        await using var dbContext = await DbContextFactory.CreateDbContextAsync();
        var existingGuids = await dbContext.OldNews
    ...
    protected async Task RemoveExistingClubs(List<ClubSql> sqlClubs)
    {
        await using var dbContext = await DbContextFactory.CreateDbContextAsync();
        var existingGuids = await dbContext.OldNews
    ...
    #region Lookups
    protected async Task<ClubLookup> CreateClubsLookup()
    {
    ...
    protected async Task<PersonLookup> CreatePersonsLookup()
    {
    ...
    protected async Task<PlaceLookup> CreatePlacesLookup()
    {
    ...
    protected async Task<TicketLookup> CreateTicketsLookup()
    {
    ...
    protected async Task<OfferLookup> CreateOffersLookup()
    {
    ...
    protected async Task<OrderLookup> CreateOrdersLookup()
    {
    ...
    protected async Task<KitLookup> CreateKitsLookup()
    {
    ...
    protected JobTitle? JobTitleFromRole(string? role)
    {
    ...
    protected string? JobTitleOtherFromRole(string? role)
    {
        if (String.IsNullOrWhiteSpace(role)) return null;
    ...
    protected string? GetYearFromKey(string key)
    {
        var match = _parseSkuKey.Match(key);
        return (match.Success) ? match.Groups["year"].Value : null;
    }
    protected string? GetExtraFromKey(string key)
    {
        var match = _parseSkuKey.Match(key);
    ...
    protected string GetFulcoIdFromSqlSky(SkuSql sqlSku)
    {
    ...
    protected static readonly List<string> GradeStrings = [
        @",\s*K\s*-\s*2nd\s*Grade",
    ...
    protected static string DropGradesFromTitle(string title)
    {
    ...
    protected static string GetKeyFromData(string year, int season, AgeLevel ageLevel, string? version) =>
    ...
    protected static string GetTitleFromKit(KitDb kit)
    {
