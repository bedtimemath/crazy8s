...
namespace SC.Audit.EFCore.Interceptors;

internal sealed class AuditInterceptor(
    ILogger<AuditInterceptor> logger,
    IDbContextFactory<SCAuditContext> dbContextFactory) : SaveChangesInterceptor, IAuditInterceptor
{
    ...
    private readonly List<DataChangeDb> _auditEntries = [];
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            foreach (var entry in eventData.Context.ChangeTracker.Entries())
            {
                ...
                {
                    dataChange.EntityId = (entry.Entity as IBaseDb)?.Id;
            ...
            // may not need to do this, but doesn't hurt
            _auditEntries.Clear();
        }
        catch (Exception ex)
        {
    ...
    private static DataChangeDb DataChangeFromEntry(EntityEntry entry, List<PropertyChangeData> properties)
    {
        ...
        {
            Identifier = (entry.Entity as IBaseDb)?.UniqueId,
            EntityId = (entry.Entity as IBaseDb)?.Id,
            EntityName = entry.Metadata.ClrType.Name,
            EntityState = entry.State,
            PropertiesJson = SerializeProperties(properties).LimitTo(SoftCrowConstants.MaxLengths.XXXLong),
            Description = description.LimitTo(SoftCrowConstants.MaxLengths.Standard),
