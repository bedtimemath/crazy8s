...
public static class StringEx
{
    ...
    #region Public Methods (Increment)
    public static string IncrementFinalDigits(this string input)
    {
        var endingMatch = SoftCrowRegex.GetMatchForEndingDigits(input);
        return (!endingMatch.Success) ? input + "1" :
    ...
    public static string? ToFirstWord(this string input, char delimiter = ' ')
    {
        if (string.IsNullOrEmpty(input?.Trim())) return input;
        input = input.Trim();
        var spaceIndex = input.IndexOf(delimiter);
        if (spaceIndex < 0) return input;
        return input.Substring(0, spaceIndex);
    }

    public static string? ToNonFirstWords(this string input, char delimiter = ' ')
    {
        if (string.IsNullOrEmpty(input?.Trim())) return input;
        input = input.Trim();

        var firstWord = input.ToFirstWord(delimiter);
    ...
    public static string RemoveNonAlphanumericWithSpaces(this string input)
    {
        ...
        input = input.Replace('.', ' ');
        input = input.Replace('-', ' ');
        input = SoftCrowRegex.ReplaceSpaces(input);
    ...
    public static string RemoveNonAlphanumeric(this string input, string replacement)
    {
        return SoftCrowRegex.ReplaceNonAlphanumeric(input, replacement);
    }

    public static string SplitCamelCase(this string input)
    {
        return SoftCrowRegex.ReplaceCapitals(input).Trim();
    }
    ...
    #region Public Methods (Json)
    
    public static JsonElement ConvertToJsonElement(this string input)
    {
        try
        {
        ...
        {
            // If parsing fails, create a JsonElement with "Body" and the raw input string
            var json = $"{{\"RawInput\": \"{input.Replace("\"", "\\\"")}\"}}";
